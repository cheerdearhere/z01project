	...
	$.ajax({
		...
		success: function(result){
			$.each(result.datas,function(index, obj){
				chartLabels.push(obj.opState);
				chartData1.push(obj.opQuanty);
			});
			createChart();
			document.getElementById("orderStateCurrent").classList.remove("btn-outline-info");
			document.getElementById("orderStateCurrent").classList.add("btn-info");
			document.getElementById("orderStateCurrent").setAttribute("disabled","disabled");
		},
		error: function(){
			alert("order state pie graph: 서버접속 실패");
		}
	});
	//another graphs
