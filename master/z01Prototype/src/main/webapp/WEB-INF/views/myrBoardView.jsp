			...
			<c:forEach items="${listContent}" var="dto">
				<tr>
					...
					<td>
						...
						<a class="contentView" href="myrContentView?rbId=${dto.rbId}">${dto.rTitle}</a>
					</td>
					<td>${dto.uEmail}</td> 
					<td>${dto.rDate}</td> 	
			    </tr>	
...
$(function(){	
	...
	$("a.contentView").click(function(event){
		...
		let contentTitle= ceo.parent().prev();
		let cword=contentTitle.text();
		contentTitle="myReview(제품번호: "+cword+")";
		$.ajax({
			url : ceo.attr("href"),
			type : "get",
			data : "",
...
$("#searchInput").on("focus",function(){
	console.log('focus');
	$.ajax({
		url : "mySearchBoard?uEmail="+'<c:out value="${user_id}"/>',
		type : "get",
		data : "",
		success : function(data) {
			$("#managerBoard").html(data);
			$("#infotxt").text("리뷰 게시판 검색 입니다.");
		},
		error : function() {
			alert("서버 접속 실패");
		}
